...
Indflix/
  ...
  tools/                           live probe harnesses (site contract checkers)
    mm_probe.py                    Multimovies wire-shape probe
    server_probe.py                IndStream embed-server probe
...
- Unit tests for all modules live in `src/test/kotlin/Test/` (package `Test`), importing `com.multimovies` or `com.indstream` classes.
