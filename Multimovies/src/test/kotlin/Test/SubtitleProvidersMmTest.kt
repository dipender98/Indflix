...
import com.multimovies.SubtitleServices
import com.multimovies.SubtilesProvider
import com.multimovies.SubtilesProvider.SubTrack
...
class SubtitleProvidersMmTest {
    ...
    fun buildUrl_nullSeason_movie_pipesEncoded() {
        val url = SubtilesProvider.buildUrl("tt1375666", null, null, setOf("Hindi", "English"))
    ...
    @Test
    fun buildUrl_series_pathAndCodes_seasonWithoutEpisodeIsMovie() {
        ...
        val partial = SubtilesProvider.buildUrl("tt0944947", 2, null, setOf("Hindi"))
        assertTrue(partial.endsWith("/subtitles/movie/tt0944947.json"), partial)
    }

    @Test
    fun buildUrl_emptyLangs_fallsBackToEnHi() {
        val url = SubtilesProvider.buildUrl("tt1375666", null, null, setOf("Klingon"))
    ...
    @Test
    fun subSenseUrl_seasonWithoutEpisode_staysMovie() {
        val url = SubtilesProvider.subSenseUrl("tt1375666", 2, null, setOf("English"))
        assertTrue(url.endsWith("/subtitles/movie/tt1375666.json"), url)
    }
...
// canonicalName.
    ...
    fun parseSubSense_langFallsBackToIdToken() {
        ...
        assertEquals(SubtitleServices.canonicalName("tam"), tracks[0].lang)
    }
...
// duplicate English url collapses.
    ...
    // native-script canonicalisation + menu labels.

    @Test
    fun canonicalName_nativeScriptsAndCodesRoundTrip() {
        assertEquals("Hindi", SubtitleServices.canonicalName("\u0939\u093F\u0928\u094D\u0926\u0940"))
        assertEquals("Tamil", SubtitleServices.canonicalName("\u0BA4\u0BAE\u0BB4\u0BCD"))
        assertEquals("Telugu", SubtitleServices.canonicalName("\u0C24\u0C46\u0C32\u0C41\u0C17\u0C41"))
        assertEquals("Malayalam", SubtitleServices.canonicalName("\u0D2E\u0D32\u0D2F\u0D3E\u0D33\u0D02"))
        assertEquals("Kannada", SubtitleServices.canonicalName("\u0C95\u0CA8\u0CCD\u0CA8\u0CA1"))
        assertEquals("Marathi", SubtitleServices.canonicalName("\u092E\u0930\u093E\u091F\u0940"))
        assertEquals("Punjabi", SubtitleServices.canonicalName("\u0A2A\u0A70\u0A1C\u0A3E\u0A2C\u0A40"))
        assertEquals("Gujarati", SubtitleServices.canonicalName("\u0A97\u0AC1\u0A9C\u0AB0\u0ABE\u0AA4\u0AC0"))
    }
