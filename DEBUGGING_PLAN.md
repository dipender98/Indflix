...
| # | Symptom |
|---|---------|
| S1 | Tapping a title and opening the links screen ends in no-link-found. |
| S2 | Detail page shows no metadata (no plot/score/backdrop); user directive: if CineVood supports IMDb use it, or Simkl or any other source. |

All file:line anchors are from current HEAD (9b1e94c).

## 1. Pipeline map (what to instrument)

\CineVoodPlugin.kt
  load(L152)             post JSON -> TitleParser -> groups -> TMDB -> response
    groups parse L161    PostParser.parseDownloadGroups(post.content)
    tmdbLookup L167      returns null while TMDB_API_KEY empty (SharedServices.kt:17)
    movie/tv resp L192/L200   plot via cleanPlot(L227) may strip excerpt to null
  loadLinks(L232)        data postUrl|season -> postByLink -> groups
    quality gate L238    .filter { it.quality >= MIN_QUALITY=720 } (L330)
    parallel fan-out     every group -> LinkExtractors.emit
CineVoodGate.kt
  killer L32             EAGER CloudflareKiller() at provider construction
  resolve(L41)           4x app.get(interceptor=killer, timeout=45)
    L55-57               ANY exception -> return null, SILENTLY (no log)
    L58-67               meta-refresh only; Just-a-moment body just re-loops
LinkExtractors.kt
  emit(L27) -> dispatch(L39)
    L50 drive direct     usercontent build
    L65 direct media     .mkv/.mp4/.m3u8 passthrough
    L72-80 gdflix page   plain app.get WITHOUT killer (CF page -> no anchors)
    L96 tagged           core loadExtractor callback (non-suspend ctor)
SiteApi.kt
  postByLink(L193)       /wp-json/wp/v2/posts?slug=&_embed (one call, 15-min cache)
  attempt(L64)           mirror rotation, exceptions swallowed into rotate
SharedServices.kt
  tmdbLookup(L52)        early-return null on empty key
\
Device capture: adb logcat -s CineVood after F0 lands (below).

## 2. S1 no-link-found -- root-cause candidates

Ranked most-likely-first. F0 diagnostics turn each into confirmed/refuted;
every path into emitted==0 must print WHY.

- RC-1A (HIGH) gate never solves. killer is built EAGERLY at provider
  construction (CineVoodGate.kt:32); CloudflareKiller must be created lazily
  (WebView/app-context readiness). A dead killer means every
  mobilejsr.rest/genxfm... returns the 403 challenge, the loop at L45 burns
  4 silent tries and returns null; every button on every post is gated, so
  emitted==0 matches S1 exactly. usesWebView is NOT overridden (provider
  flags L42-48; MainAPI default false) -- the app may also skip
  WebView-assisted paths.
  FIX F1: by lazy { CloudflareKiller() }; override usesWebView = true;
  DIAG per attempt: GATE try#n status= code= cf=<challenge?> final=<url>.

- RC-1B (HIGH) 720p floor wipes the post. The L238 filter drops every group
  below 720p; CAM/HDTC re-releases and some series packs ship 480p-only ->
  zero groups -> clean false with no signal.
  FIX F2: if the filtered list is empty but raw groups exist, keep ALL
  groups (still quality-labelled in the server list); DIAG GROUPS raw=N hd=M.

- RC-1C (MED) gdflix branch fetches unprotected. LinkExtractors.kt:73 uses a
  plain app.get (no killer) -- a CF page yields zero anchors and the generic
  loadExtractor fallback then sees only the challenge.
  FIX F3: route the file-page fetch through the gates solved cookie jar
  (pass the CineVoodGate/fetcher into emit), classify Just-a-moment bodies.

- RC-1D (MED) zero groups parsed. content.rendered markup drift (new post
  template / adult-blur variants) breaks the .mfx-download-group scan; the
  flat fallback (PostParser.kt:34-44) only fires when hrefs contain
  genx|mobilejsr.
  FIX F4: widen fallback to any anchor whose ancestor is a download wrapper;
  DIAG prints group count + first href on zero.

- RC-1E (LOW-MED) JS-only redirect. Gate may land on window.location /
  location.replace hops; only meta refresh is followed (L23-29) -> still
  on-gate after 4 loops -> null.
  FIX F5: also extract window.location(.href)?= and location.replace() hops.

- RC-1F (LOW) season pick empties. Episode data url|N filters groups to one
  season; posts that mislabel seasons can produce an empty pick.
  FIX: empty pick -> all groups + DIAG.

- RC-1G (LOW) device-side wp-json block. attempt(L64) rotates mirrors
  silently; a carrier-level block of the whole network surfaces as an app
  error (not S1) but must still log. DIAG per rotation: MIRROR now=...

## 3. S2 empty detail metadata -- root causes + metadata-source decision

- RC-2A (CERTAIN) TMDB key never shipped. TMDB_API_KEY is empty
  (SharedServices.kt:17) -> tmdbLookup early-returns null -> no score,
  backdrop, genres, year-correction. Known shipped-degraded state.
- RC-2B (HIGH) plot is self-sabotaged. Excerpts literally begin with
  Download ...; cleanPlot (CineVoodPlugin.kt:227) strips exactly that ->
  sub-20-char -> null plot on most posts. The real synopsis sits in
  .mfx-plot-box inside content.rendered and is never read.
- RC-2C (MED) poster single-sourced. Poster comes only from
  _embedded.wp:featuredmedia; posts without featured media render bare.
- RC-2D (LOW) all-or-nothing load. Any exception inside the load attempt
  block kills metadata AND links together.

### Metadata source decision (user directive: IMDb-first, Simkl or any other)

| Source | Keyless | IMDb-id keyed | Verdict |
|---|---|---|---|
| Cinemeta (v3-cinemeta.strem.io/meta/{movie|tv}/{tt}.json) | yes | yes | CHOSEN: poster/background/description/year/genres/runtime/imdbRating in one call; /search/{type}/{q}.json fallback when a post has no IMDb link |
| TMDB (existing path) | no | find-by-imdb needs key | keep as optional override when key present |
| Simkl | no (client_id) | partial | rejected: client_id + OAuth |
| OMDb | no (key) | yes | rejected: key |
| Trakt | no (OAuth) | yes | rejected: OAuth |

CineVood posts embed their own IMDb link (.mfx-imdb a, parsed by
PostParser.extractImdbId, PostParser.kt:21) -- the source the user pointed at.

- FIX F6 MetadataPicker: chain = Cinemeta-by-imdbId -> Cinemeta-search
  (name+year disambiguation) -> site-only. Maps description->plot,
  poster->posterUrl, background->backgroundPosterUrl, genres->tags,
  runtime->duration, imdbRating->Score.from10; fills syncData imdb_id
  (+tmdb when key exists) so library sync works. Pure JSON parsing ->
  offline-testable on a committed fixture.
- FIX F7 plot = .mfx-plot-box text (new PostParser.parsePlot) -> excerpt
  (tag-strip only, no download-strip) -> Cinemeta description.
- FIX F8 poster fallback chain: featured media -> Cinemeta -> first
  .mfx-shot img from content.

## 4. Instrumentation + evidence steps (F0, before any fix)

1. DIAG (com.lagradost.api.Log, tag CineVood) at: postByLink (bytes,
   _embedded presence, imdb id), parseDownloadGroups (raw count), quality
   gate (before/after), gate resolve (every attempt), dispatch (stage
   chosen, emit url+quality+audio), search fallback usage, mirror rotation.
2. Extend tools/cv_probe.py: --post <slug> prints JSON status, _embedded
   presence, IMDb id, group labels + gate hosts, hd(>=720p) count;
   --gate <url> classifies OK / CF-MANAGED / TLS.
3. Reproduce on device with logcat; capture one movie (Toxic), one series
   (Chumbak S1), one 480p-only post.
4. Commit a Cinemeta fixture (tools/fixtures/cinevood/cinemeta_tt0813715.json)
   once fetched, for offline tests.

## 5. Acceptance criteria (close when all green)

1. Toxic (movie): links screen non-empty, every link labelled
   quality+audio; detail page shows poster, plot, year, score, genres.
2. Chumbak S1: season row appears; links resolve through the gate.
3. A 480p-only post: links appear via F2 fallback (or an explicit DIAG
   reason) -- never a silent no-link-found.
4. Gate-unresolvable case: DIAG prints the failing stage; any direct links
   still surface.
5. CineVoodParserTest extended (480p fallback, plot-box parse, Cinemeta
   parse); cv_probe --post green on 3 live posts; bump CineVood v1 -> v2.
