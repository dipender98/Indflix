...
/** FILE: VidLinkSource. kt â€” the VidLink third-party stream source (vidlink. pro). - token generation. (XSalsa20-Poly1305 secretbox over mediaId +. */

import java.math.BigInteger


/** VidLink (vidlink. pro) token generator + API client. The site's own player encrypts `{mediaId}{unix+480s}` with. XSalsa20-Poly1305 (nacl. */
object VidlinkSource {
    ...
    /** Playback headers for one quality entry. Base is (native UA only - the mwVault CDN 429s on any Referer). */
    ...
    internal fun token(mediaId: String, timestampSeconds: Long): String {
        ...
        // 64-bit big-endian timestamp (. setUint32 high/low).
        for (i in 0 until 8) {
...
/** Minimal pure-Kotlin). All byte order is little-endian; Salsa20 arithmetic uses 32-bit Int with unsigned shifts. */
internal object NaCl {
    ...
    /** 20-round Salsa20 core. = full core (adds input back); otherwise returns the raw final state (HSalsa20 behavior). */
    ...
    /** HSalsa20: derive a 32-byte subkey. */
    ...
    /** XSalsa20 keystream of bytes for a 24-byte nonce + 32-byte key. */
    ...
    /** Poly1305 MAC with = 32-byte one-time key over. */
    ...
    // ── secretbox ─────────────────────────────────────────────────────── NaCl secretbox (XSalsa20-Poly1305): returns.
// MAC(16) || ciphertext, matching.
    internal fun secretbox(message: ByteArray, nonce24: ByteArray, key: ByteArray): ByteArray {
        require(nonce24.size == 24 && key.size == 32)
        // TweetNaCl pads the message with 32 zero bytes; the first keystream 32 bytes then double as the Poly1305 key.
